...
export async function GET(request: NextRequest) {
  try {
    ...
    const getMaxStockValue = (sheetData: ItemData[], maxDay: number): number => {
      let max = 0;
    ...
    for (const dataType of dataTypes) {
      ...
      const monthName = MONTH_NAMES[validatedQuery.month - 1];
      const reportTypeName = validatedQuery.type === "raw-materials" ? "BAHAN BAKU" : "PRODUK JADI";
      ...
      titleCell.font = { size: 14, bold: true, color: { argb: "FF" + dataType.color } };
          ...
          if (colNumber <= 2) {
            ...
            if (typeof cell.value === "number") {
              ...
              if (dataType.key === "stok-sisa") {
                ...
                } else if (cell.value > 0 && cell.value <= lowStockThreshold && lowStockThreshold > 0) {
              ...
              } else if (dataType.key === "stok-masuk" || dataType.key === "stok-keluar") {
                // For movement sheets: highlight high activity (>100 units)
                if (cell.value >= 100) {
                  cell.font = { size: 10, bold: true, color: { argb: "FF" + dataType.color } };
                }
              }
              ...
              if (cell.value < 0) {
                ...
                cell.font = { size: 10, color: { argb: "FFB91C1C" }, bold: true };
              }
            }
          }
        });
