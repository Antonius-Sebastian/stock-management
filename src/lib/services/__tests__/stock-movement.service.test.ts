...
describe('Stock Movement Service', () => {
  ...
  describe('createStockMovement', () => {
    it('should create IN movement for raw material and update stock', async () => {
      ...
      const mockTx = {
        ...
        drum: {
          findUnique: vi.fn().mockResolvedValue({ currentQuantity: 100 }),
          update: vi.fn(),
          aggregate: vi.fn().mockResolvedValue({
            _sum: { currentQuantity: 110 },
          }),
        },
        finishedGood: {
          findUnique: vi.fn(),
      ...
      vi.mocked(prisma.$transaction).mockImplementation(
        ...
        async (callback: any) => {
          console.log('MockTx keys:', Object.keys(mockTx))
          // @ts-ignore
          if (mockTx.finishedGoodStock) console.log('finishedGoodStock keys:', Object.keys(mockTx.finishedGoodStock))
    ...
    it('should throw error when insufficient stock for OUT movement', async () => {
      ...
      // Mock groupBy to return 20 stock (insufficient for 150)
      vi.mocked(prisma.stockMovement.groupBy).mockResolvedValue([
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        { type: 'IN', _sum: { quantity: 20 } } as any,
      ])
    ...
    it('should throw error when negative ADJUSTMENT would result in negative stock', async () => {
      ...
      // Mock groupBy to return 20 stock
      vi.mocked(prisma.stockMovement.groupBy).mockResolvedValue([
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        { type: 'IN', _sum: { quantity: 20 } } as any,
      ])
    ...
    it('should create movement for finished good', async () => {
      ...
      expect(result).toEqual(mockMovement)
      // Expect upsert instead of global update
  ...
  describe('deleteStockMovementsByDate', () => {
    ...
    it('should delete movements and restore stock for finished good', async () => {
      ...
      expect(mockTx.finishedGoodStock.update).toHaveBeenCalledWith({
        where: {
